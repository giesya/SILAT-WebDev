...
const SAMPLE_VESSELS_PENCABUTAN = [
  {
    id: 1,
...
export default function IKPencabutan({ onLogout }) {
  // Form input states
  ...
  const filteredVessels = useMemo(() => {
    ...
    if (!q) return SAMPLE_VESSELS_PENCABUTAN
    return SAMPLE_VESSELS_PENCABUTAN.filter(
      (item) =>
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!vesselName.trim() || !revocationDate || !revocationReason.trim()) {
  ...
  const handleConfirmRevocation = () => {
    ...
    setVesselStatus('Dicabut')
    setToast({
      type: 'success',
      message: `Izin kapal ${vesselName} (${licenseNo || 'SIPI/SIKPI'}) milik ${companyName || 'Perusahaan'} berhasil DICABUT pada tanggal ${revocationDate}.`,
    })
  }
  ...
  const breadcrumbs = [
    ...
    { label: 'Pencabutan Izin Kapal' },
  ]

  return (
    <Layout
      currentPath="/layanan/pencabutan/ik"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      title="Pencabutan Izin Kapal"
    >
      {/* Toast Notification */}
      {toast && (
      ...
      <section className="w-full bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] p-6 md:p-8 shadow-xs mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-[var(--color-text)]">
                Formulir Pencabutan Izin Kapal (SIPI / SIKPI)
              </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Row 7: Tanggal Pencabutan * (Left) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {/* 13. Tanggal Pencabutan * */}
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1.5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Masukkan dasar penetapan pencabutan izin kapal (misal: permohonan penghapusan kapal / pengalihan kepemilikan / kapal afkir atau tenggelam)..."
              className="w-full px-4 py-2.5 rounded-xl border border-[rgba(31,78,120,0.15)] bg-[var(--color-bg)] text-xs font-medium text-[var(--color-text)] focus:outline-none focus:border-[var(--color-primary)]"
            />
          </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 text-rose-600 border-b pb-3">
              ...
              <div>
                <h3 className="font-bold text-base text-[var(--color-text)]">
                  Konfirmasi Pencabutan Izin Kapal
                </h3>
            ...
            <div className="space-y-2.5 text-xs text-slate-700 p-4 bg-rose-50/60 border border-rose-200/50 rounded-xl">
              <p>
                Anda akan melakukan pencabutan izin kapal secara permanen terhadap:
              </p>
              <div className="font-semibold text-slate-800 space-y-1">
                <p>• Nama Kapal: <span className="font-bold">{vesselName || '-'}</span></p>
                <p>• Nomor Izin: <span className="font-mono text-rose-700">{licenseNo || '-'}</span></p>
