...
const SAMPLE_PERMOHONAN_SIUP = [
  {
    id: 'PMH-SIUP-2026-001',
...
export default function IUDaftarPermohonan({ onLogout }) {
  // Search form states
  ...
  const filteredData = useMemo(() => {
    return SAMPLE_PERMOHONAN_SIUP.filter((item) => {
      // Nama Pemohon filter
  ...
  const breadcrumbs = [
    { label: 'Layanan' },
    { label: 'Permohonan' },
    { label: 'Daftar Permohonan SIUP' },
  ]

  return (
    <Layout
      currentPath="/layanan/permohonan/iu/daftar-permohonan"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      title="Daftar Permohonan SIUP"
    >
      {/* Form Pencarian */}
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] p-6 shadow-xs mb-8">
        <h2 className="text-base font-bold text-[var(--color-text)] mb-4 flex items-center gap-2">
          ...
          <span>Form Pencarian Permohonan SIUP</span>
        </h2>
        ...
        <form onSubmit={handleSearch} className="space-y-4">
          ...
          {/* Row 2: Tanggal Permohonan s/d, Buttons (tanpa form verifikator) */}
      ...
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] shadow-xs overflow-hidden mb-8">
        ...
        {sortedData.length === 0 ? (
          <div className="p-12 text-center text-sm text-[var(--color-muted)]">
            Data permohonan SIUP tidak ditemukan.
          </div>
        ) : (
          <Pagination
