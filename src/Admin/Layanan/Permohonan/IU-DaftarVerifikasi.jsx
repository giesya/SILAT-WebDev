...
const VERIFIKASI_DATA = {
  ...
  penolakan: [
    {
      id: 'TOLAK-SIUP-2026-001',
      applicant: 'CV Samudra Mas',
      person: 'Gunawan Prasetyo',
      type: 'SIUP',
      status: 'Baru',
      verifier: 'Dimas',
      date: '2026-01-14',
      displayDate: '14 Jan 2026',
      alasan: 'Dokumen NIB dan NPWP tidak terdaftar valid pada OSS',
    },
    {
      id: 'TOLAK-SIUP-2026-002',
      applicant: 'PT Pasifik Jaya Mina',
      person: 'Rudi Hartono',
      type: 'SIUP',
      status: 'Perpanjangan',
      verifier: 'Sara Kristiana L',
      ...
      alasan: 'Masa berlaku dokumen izin lingkungan telah kadaluarsa',
    },
    {
      id: 'TOLAK-SIUP-2026-003',
      applicant: 'CV Bahari Perkasa Sentosa',
      person: 'Yuliana',
      type: 'SIUP',
      status: 'Perubahan',
      verifier: 'Tiara Dwi M',
      ...
      alasan: 'Surat pernyataan tidak sesuai format baku KKP',
    },
  ],
}
...
export default function IUDaftarVerifikasi({ onLogout }) {
  const [activeTab, setActiveTab] = useState('permohonan')

  // Search form states (same as DAFTAR PENDOK SIUP)
  ...
  const filteredData = useMemo(() => {
    const list = VERIFIKASI_DATA[activeTab] || []
  ...
  const breadcrumbs = [
    { label: 'Layanan' },
    { label: 'Permohonan' },
    { label: 'Daftar Verifikasi SIUP' },
  ]

  return (
    <Layout
      currentPath="/layanan/permohonan/iu/daftar-verifikasi"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      title="Daftar Verifikasi Izin Usaha"
    >
      {/* Form Pencarian (Sama seperti DAFTAR PENDOK SIUP) */}
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] p-6 shadow-xs mb-8">
        <h2 className="text-base font-bold text-[var(--color-text)] mb-4 flex items-center gap-2">
          ...
          <span>Form Pencarian Verifikasi SIUP</span>
        </h2>
      ...
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] shadow-xs overflow-hidden mb-8">
        ...
        {sortedData.length === 0 ? (
          <div className="p-12 text-center text-sm text-[var(--color-muted)]">
            Data verifikasi SIUP tidak ditemukan.
          </div>
        ) : (
          <Pagination
