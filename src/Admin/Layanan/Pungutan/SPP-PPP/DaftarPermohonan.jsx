...
export default function SPPPPPDaftarPermohonan({ onLogout }) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!q) return SAMPLE_PERMOHONAN_PPP
    return SAMPLE_PERMOHONAN_PPP.filter((item) =>
  ...
  return (
    <Layout
      currentPath="/layanan/pungutan/spp-ppp/daftar-permohonan"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      title="Daftar Permohonan SPP–PPP"
    >
      {/* Search Bar Section */}
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] p-6 shadow-xs mb-8">
        <h2 className="text-base font-bold text-[var(--color-text)] mb-3">Pencarian Permohonan SPP–PPP</h2>
      ...
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] shadow-xs overflow-hidden mb-8">
        ...
        {sortedData.length === 0 ? (
          <div className="p-12 text-center text-sm text-[var(--color-muted)]">
            Data permohonan SPP–PPP tidak ditemukan.
          </div>
        ) : (
          <Pagination
          ...
          >
            ...
            <div className="space-y-3 text-xs text-slate-700">
              ...
              <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-emerald-800 font-bold">Nominal Pungutan (PPP):</span>
