...
export default function IUDaftarIzinUsaha({ onLogout }) {
  // Form search states
  ...
  const filteredData = useMemo(() => {
    return SAMPLE_DAFTAR_SIUP.filter((item) => {
  ...
  const breadcrumbs = [
    { label: 'Pencetakan' },
    { label: 'Izin Usaha' },
    { label: 'Daftar Izin Usaha' },
  ]

  return (
    <Layout
      currentPath="/layanan/pencetakan/iu-daftar"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      title="Daftar Izin Usaha (SIUP)"
    >
      ...
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] p-6 shadow-xs mb-8">
        <h2 className="text-base font-bold text-[var(--color-text)] mb-4">
          Pencarian Daftar Izin Usaha (SIUP)
        </h2>
      ...
      <section className="bg-white rounded-2xl border border-[rgba(31,78,120,0.08)] shadow-xs overflow-hidden mb-8">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h3 className="font-bold text-sm text-[var(--color-text)] flex items-center gap-2">
            ...
            <span>Hasil Daftar Izin Usaha (SIUP) ({sortedData.length} data ditemukan)</span>
          </h3>
        </div>
        ...
        {sortedData.length === 0 ? (
          <div className="p-12 text-center text-sm text-[var(--color-muted)]">
            Data izin usaha (SIUP) tidak ditemukan. Silakan gunakan filter pencarian di atas.
          </div>
        ) : (
          <Pagination
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-base text-[var(--color-text)]">
                    Pratinjau Dokumen Izin Usaha (SIUP)
                  </h3>
