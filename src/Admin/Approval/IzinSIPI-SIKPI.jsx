...
const SAMPLE_SIPI_DATA = [
  { id: 'SIPI-2026-001', vessel: 'KM. Bahari 01', owner: 'PT Laut Nusantara', gt: '120 GT', type: 'SIPI', stage: 'Katimja', date: '12 Jan 2026' },
  { id: 'SIPI-2026-002', vessel: 'KM. Samudra 08', owner: 'CV Samudra Jaya', gt: '85 GT', type: 'SIPI', stage: 'Direktur', date: '15 Jan 2026' },
  { id: 'SIKPI-2026-001', vessel: 'KM. Maju Bahari', owner: 'PT Bahari Maju', gt: '180 GT', type: 'SIKPI', stage: 'Approved', date: '20 Jan 2026' },
]

export default function ApprovalIzinSIPISIKPI({ onLogout }) {
  const [activeTab, setActiveTab] = useState('Katimja')
  const [searchQuery, setSearchQuery] = useState('')
  const [data, setData] = useState(SAMPLE_SIPI_DATA)
  ...
  const breadcrumbs = [
    { label: 'Approval' },
    { label: 'Approval Izin SIPI & SIKPI' },
  ]

  return (
    <Layout
      currentPath="/approval/izin-sipi-sikpi"
      onLogout={onLogout}
      breadcrumbs={breadcrumbs}
      eyebrow="Approval Perizinan Kapal"
      title="Approval Izin SIPI & SIKPI"
      subtitle="Persetujuan bertingkat penerbitan Surat Izin Penangkapan Ikan (SIPI) dan Surat Izin Kapal Pengangkut Ikan (SIKPI)."
    >
      {toast && (
      ...
      <div className="bg-white p-4 sm:p-5 rounded-2xl border border-[rgba(31,78,120,0.08)] shadow-xs mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            placeholder="Cari permohonan kapal..."
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-[var(--color-primary)] text-white text-xs uppercase tracking-wider">
                <th className="px-5 py-3.5 font-bold">No. Izin</th>
                <th className="px-5 py-3.5 font-bold">Nama Kapal & GT</th>
                <th className="px-5 py-3.5 font-bold">Pemilik / Perusahaan</th>
                ...
                <tr key={item.id} className="hover:bg-[rgba(0,90,156,0.03)] transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5 font-bold text-slate-800">
                      ...
                      <span>{item.vessel} ({item.gt})</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 font-bold text-[var(--color-text)]">{item.owner}</td>
                  <td className="px-5 py-4">
